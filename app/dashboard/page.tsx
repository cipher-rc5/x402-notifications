    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.pricingModel) {
          setPricingModel({ model: data.pricingModel.pricing_model, plan: data.subscription });
        }
      }
    } catch (error) {
  ...
  async function handlePlanSelected(model: 'pay-per-use' | 'subscription', planId?: string, customPrice?: number) {
    try {
      const response = await fetch('/api/subscriptions', {
        ...
        body: JSON.stringify({ userId, pricingModel: model, planId, perNotificationPrice: customPrice })
      });
  ...
  async function handleMarkRead(notificationId: string) {
    try {
      ...
      setNotifications((prev) =>
        prev.map((n) => n.id === notificationId ? { ...n, status: 'read' as const, read_at: Math.floor(Date.now() / 1000) } : n)
      );
    ...
    <div className='min-h-screen bg-background'>
      <div className='container mx-auto p-6 space-y-8 max-w-7xl'>
        ...
        <div className='space-y-2'>
          ...
          <p className='text-muted-foreground text-pretty'>Manage your notifications and track payments across Solana and EVM networks</p>
        </div>
            ...
            <div className='flex items-center justify-between'>
              <div className='space-y-1'>
                <p className='text-sm font-semibold text-foreground'>
                  Current Plan: {pricingModel.model === 'subscription' ? pricingModel.plan?.plan_name || 'Subscription' : 'Pay-Per-Use'}
                </p>
                {pricingModel.model === 'subscription' && pricingModel.plan && (
                  <p className='text-xs text-muted-foreground'>
                    {pricingModel.plan.notifications_used || 0} / {pricingModel.plan.notification_limit || '∞'} notifications used
                  </p>
                )}
              </div>
              <Button
        ...
        {showPricingSelector && (
          <Card className='p-6 border-2 border-primary/50 bg-card/80 backdrop-blur-xl shadow-[0_20px_50px_rgba(220,31,255,0.2)]'>
            ...
            <Button variant='ghost' className='w-full mt-4 text-muted-foreground' onClick={() => setShowPricingSelector(false)}>
              Cancel
            </Button>
          </Card>
        )}
            ...
            <Card className='p-6 bg-card/50 border-border backdrop-blur'>
              <div className='space-y-3'>
                ...
                <div className='text-sm text-muted-foreground space-y-2 text-pretty'>
                  <p>
                    After purchasing a custom notification for $0.99 USDC, you'll receive a unique MCP endpoint that can be integrated with
                ...
                <Button variant='outline' size='sm' asChild className='border-border text-foreground hover:bg-muted bg-transparent'>
