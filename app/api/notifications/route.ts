  ...
  try {
    ...
    if (!isConnected) {
      return NextResponse.json(
        {
          error: "Database connection failed. Please check your TURSO_DATABASE_URL and TURSO_AUTH_TOKEN.",
        },
        { status: 500 },
      )
    }
    ...
    if (!hasNotificationsTable) {
      console.error("[v0] notifications table does not exist!")
      return NextResponse.json(
        {
          error:
            "Database not initialized. Please run the SQL scripts in the scripts/ folder to create the required tables.",
