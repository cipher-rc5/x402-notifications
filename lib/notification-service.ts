...
export async function canSendNotification(
  ...
  try {
    ...
    const prefResult = await turso.execute({
      sql: "SELECT * FROM user_pricing_preferences WHERE user_id = ?",
      args: [userId],
    })
    ...
    if (pref.pricing_model === "pay-per-use") {
      ...
      return {
        ...
        amount: Number(pref.per_notification_price) || 0.99,
      }
    }
...
export async function recordNotificationUsage(
  ...
  try {
    ...
    const prefResult = await turso.execute({
      sql: "SELECT pricing_model FROM user_pricing_preferences WHERE user_id = ?",
      args: [userId],
    })
