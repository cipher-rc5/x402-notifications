...
import { turso, type Notification } from "./turso"

const clientId = process.env.NOTIFICATIONAPI_CLIENT_ID
const clientSecret = process.env.NOTIFICATIONAPI_CLIENT_SECRET

if (!clientId || !clientSecret) {
  console.warn("[v0] NotificationAPI credentials missing - notifications will be logged to database only")
}

// Initialize NotificationAPI with credentials
if (clientId && clientSecret) {
  console.log("[v0] Initializing NotificationAPI client...")
  notificationapi.init(clientId, clientSecret)
  console.log("[v0] NotificationAPI client initialized")
}
...
export async function sendNotification(params: SendNotificationParams) {
  ...
  try {
    if (clientId && clientSecret) {
      console.log("[v0] Sending notification via NotificationAPI...")
      ...
      console.log("[v0] NotificationAPI response:", response)
    } else {
      console.log("[v0] Skipping NotificationAPI send (credentials not configured)")
    }

    console.log("[v0] Inserting notification into database...")
    ...
    console.log("[v0] Notification logged successfully, rows affected:", insertResult.rowsAffected)
    return { success: true, notificationId }
  } catch (error) {
    console.error("[v0] Error sending notification:", error)

    if (error instanceof Error && error.message.includes("no such table")) {
      console.error("[v0] CRITICAL: notifications table does not exist in database!")
      console.error("[v0] Please run the database initialization scripts in the scripts folder")
    ...
    try {
      ...
      console.error("[v0] Could not log failed notification to database:", dbError)
    }
...
export async function getUserNotifications(userId: string): Promise<Notification[]> {
  try {
    console.log("[v0] Querying notifications for user:", userId)
    const result = await turso.execute({
      sql: "SELECT * FROM notifications WHERE user_id = ? ORDER BY created_at DESC",
      args: [userId],
    })

    console.log("[v0] Found", result.rows.length, "notifications")
    return result.rows as unknown as Notification[]
  } catch (error) {
    console.error("[v0] Error fetching notifications from database:", error)

    if (error instanceof Error && error.message.includes("no such table")) {
      console.error("[v0] CRITICAL: notifications table does not exist!")
