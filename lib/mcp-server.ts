...
import { sendNotification, getUserNotifications } from "./notification-service"
...
async function validateSession(token: string): Promise<string | null> {
  try {
    const result = await turso.execute({
      sql: "SELECT user_id, expires_at FROM mcp_sessions WHERE session_token = ?",
      args: [token],
    })
...
export async function handleMCPRequest(request: MCPRequest): Promise<MCPResponse> {
  ...
  try {
    ...
    console.error("[v0] MCP request error:", error)
    return {
      success: false,
...
async function handleCreateSession(params: Record<string, any>): Promise<MCPResponse> {
  ...
  return {
    success: true,
    data: { sessionToken: token, expiresIn: 86400 },
...
async function handleGetTotalSpent(params: Record<string, any>): Promise<MCPResponse> {
  ...
  return {
    success: true,
    data: { totalSpent: total, currency: "USDC" },
    timestamp: Math.floor(Date.now() / 1000),
  }
}
