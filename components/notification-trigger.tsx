  ...
  const handleSendNotification = async () => {
    ...
    try {
      const response = await fetch('/api/notifications', {
        ...
        body: JSON.stringify({ userId, email: userEmail, subject, message, type })
      });
  ...
  return (
    <Card className='p-6 space-y-4 border-primary/20 bg-primary/5'>
      ...
      <p className='text-sm text-muted-foreground'>
        Trigger a notification manually to test your integration. The notification will be sent via NotificationAPI and logged to the
        database.
      </p>
