...
import { Sparkles, CheckCircle2, AlertCircle, Wallet } from "lucide-react"
    ...
    try {
      ...
      const paymentResponse = await fetch("/api/payments", {
        ...
        body: JSON.stringify({
          ...
          network,
          amount: "0.99",
          resource: `custom-notification-${Date.now()}`,
        }),
      })
    ...
    <Card className="p-6 space-y-6 border-primary/20 bg-gradient-to-br from-primary/5 to-primary/10">
      ...
      <div className="space-y-4">
        <div className="space-y-2">
          ...
          <Input
            id="custom-name"
            placeholder="e.g., Payment Alert System"
            value={name}
        ...
        <div className="space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            The condition that will trigger this notification via your MCP endpoint
          </p>
        </div>
        ...
        <div className="space-y-2">
          ...
          />
          <p className="text-xs text-muted-foreground">
            Available variables: {`{{amount}}, {{network}}, {{timestamp}}, {{userId}}`}
          </p>
        </div>
